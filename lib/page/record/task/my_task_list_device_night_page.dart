...
/* 我的任务-设备科日间巡查页面 */
class MyTaskListDeviceNightPage extends StatefulWidget {
  final String userId;

  MyTaskListDeviceNightPage(this.userId);

  @override
  State<StatefulWidget> createState() => MyTaskListDeviceNightPageState();
}

class MyTaskListDeviceNightPageState extends State<MyTaskListDeviceNightPage>
  ...
  void initState() {
    ...
    subscription = eventBus.on<EventUtil>().listen((event) {
      if (event.message == Config.REFRESH_DEVICE_NIGHT_LIST) {
        _refresh();
      }
    });
  }
  ...
  _fetchTasksMapSortByDate() async {
    List<TabDeviceRecordModel> taskList = await TabDeviceRecordManager()
        .queryUploadRecordsByUserid(widget.userId, 1);
...
//     List<TabDeviceRecordModel> tabDeviceRecordModelList = await TabDeviceRecordManager().queryUnoperatedOrderSort();
//     // 获取筛选
//     String dateSelectListAboutDeviceNight = await LocalStorage.get(Config.DATE_SELECT_LIST_ABOUT_DEVICE_NIGHT);
//     // 获取记录
//     setState(() {
//       for (int i = 0; i < tabDeviceRecordModelList.length; i++) {
//         bool _isThisUser = (tabDeviceRecordModelList[i].inspectorId == widget.userId); // 该用户
//         bool _isDeviceNight = (tabDeviceRecordModelList[i].inspectionType == 1); // 设备科夜
//         bool _isSelectDate = DatePickerUtils.getIsSelectDate(dateSelectListAboutDeviceNight, tabDeviceRecordModelList[i].time); // 是否为筛选的日期
...
//             _inspectionIdList.add(tabDeviceRecordModelList[i].inspectionId);
//           }
//         }
//       }
//     });
