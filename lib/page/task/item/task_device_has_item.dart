...
class TaskDeviceHasItem extends StatefulWidget {
  ...
  TaskDeviceHasItem(this.taskRecordModel, this.onWrongChanged, this.onRecordChanged);

  @override
  createState() => TaskDeviceHasItemState();
}

class TaskDeviceHasItemState extends State<TaskDeviceHasItem> {
  @override
  void initState() {
    _initData();
    super.initState();
                    ...
                    child: Row(children: <Widget>[
                      Container(
                          ...
                          decoration: BoxDecoration(
                              border: Border.all(
                                  color:
                                  (widget.taskRecordModel.recordType != 4)
                                      ? Colors.red
                                      : Colors.white,
                                  width: 0.5),
                              borderRadius: BorderRadius.circular(20),
                              color: (widget.taskRecordModel.recordType != 4)
                                  ? Colors.white
                                  : Colors.red),
                          child: GestureDetector(
                              ...
                              onTap: () async{
                                ...
                                if (widget.taskRecordModel.isUpload == 1) {
                                  if (widget.taskRecordModel.recordType == 5) {
                                    widget.onWrongChanged('本日【${widget.taskRecordModel.recordTitle} 】巡查内容已上传,不能修改');
                                  }
                                } else {
                                  setState(() {
                                    widget.taskRecordModel.recordType = 4; // 有
                      ...
                      Container(
                          ...
                          decoration: BoxDecoration(
                              border: Border.all(
                                  color:
                                  (widget.taskRecordModel.recordType != 5)
                                      ? Colors.green
                                      : Colors.white,
                                  width: 0.5),
                              borderRadius: BorderRadius.circular(20),
                              color: (widget.taskRecordModel.recordType != 5)
                                  ? Colors.white
                                  : Colors.green),
                          child: GestureDetector(
                              behavior: HitTestBehavior.opaque,
                              ...
                              onTap: () async{
                                ...
                                if (widget.taskRecordModel.isUpload == 1) {
                                  if (widget.taskRecordModel.recordType == 4) {
                                    widget.onWrongChanged('本日【${widget.taskRecordModel.recordTitle} 】巡查内容已上传,不能修改');
                                  }
                                } else {
                                  setState(() {
                                    widget.taskRecordModel.recordType = 5; // 无
                                    widget.onRecordChanged(widget.taskRecordModel);
                                    TabDeviceRecordManager().insert(widget.taskRecordModel);
                                  });
                                }
