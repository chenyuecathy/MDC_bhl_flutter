...
class TaskDeviceOpenCloseItem extends StatefulWidget {
  ...
  TaskDeviceOpenCloseItem(this.taskRecordModel, this.onWrongChanged, this.onRecordChanged);

  @override
  createState() => TaskDeviceOpenCloseItemState();
}

class TaskDeviceOpenCloseItemState extends State<TaskDeviceOpenCloseItem> {
  @override
  void initState() {
    _initData();
    super.initState();
                    ...
                    child: Row(children: <Widget>[
                      Container(
                          ...
                          decoration: BoxDecoration(
                              border: Border.all(
                                  color:
                                      (widget.taskRecordModel.recordType != 2)
                                          ? Colors.red
                                          : Colors.white,
                                  width: 0.5),
                              borderRadius: BorderRadius.circular(20),
                              color: (widget.taskRecordModel.recordType != 2)
                                  ? Colors.white
                                  : Colors.red),
                      ...
                      Container(
                          ...
                          decoration: BoxDecoration(
                              border: Border.all(
                                  color:
                                      (widget.taskRecordModel.recordType != 3)
                                          ? Colors.green
                                          : Colors.white,
                                  width: 0.5),
                              borderRadius: BorderRadius.circular(20),
                              color: (widget.taskRecordModel.recordType != 3)
                                  ? Colors.white
                                  : Colors.green),
                          child: GestureDetector(
                              behavior: HitTestBehavior.opaque,
                              ...
                              onTap: () {
                                if (widget.taskRecordModel.isUpload == 1) {
                                  if (widget.taskRecordModel.recordType == 2) {
                                    widget.onWrongChanged('本日【${widget.taskRecordModel.recordTitle} 】巡查内容已上传,不能修改');
                                  }
                                } else {
                                  setState(() {
                                    widget.taskRecordModel.recordType = 3; // 关
                                    widget.onRecordChanged(widget.taskRecordModel);
                                    TabDeviceRecordManager().insert(widget.taskRecordModel);
                                  });
                                }
  ...
  onItemClick() {
    // debugPrint(widget.taskRecordModel.toString());
  }
}
